...
class app_openweather extends module
{
   ...
   function view_weather(&$out)
   {
      ...
      if ($forecast > 0)
      {
         ...
         for ($i = 0; $i <= $forecast; $i++)
         {
            ...
            $out["FORECAST"][$i]["temp_day"] = gg('ow_day'.$i.'.temp_day');
            $out["FORECAST"][$i]["temp_eve"] = gg('ow_day'.$i.'.temp_eve');
   ...
   function get_weather($cityID)
   {
      ...
      $fact = $weather->list[0];
      $date = date("d.m.Y G:i:s T Y", $fact->dt);
     
      sg('ow_fact.temperature', app_openweather::GetCurrTemp($fact->temp));
      sg('ow_fact.weather_type', $fact->weather[0]->description);
      sg('ow_fact.wind_direction', $fact->deg);
      sg('ow_fact.wind_speed', $fact->speed);
      ...
      sg('ow_fact.image', $fact->weather[0]->icon);
      sg('ow_fact.clouds', $fact->clouds);
      ...
      foreach($weather->list as $day)
      {
         ...
         sg('ow_day'.$i.'.rain', isset($day->rain) ? $day->rain : '');
   ...
   protected static function GetJsonWeatherDataByCityID($cityID, $unit = "metric")
   {
      ...
      $query  = "http://api.openweathermap.org/data/2.5/forecast/daily?id=" . $cityID . "&mode=json&units=" . $unit . "&cnt=16&lang=ru";
    ...
    * @return array|bool
    */
   function GetSunInfoByCityID($cityID, $timeStamp = -1)
   {
      if($timeStamp == '' or $timeStamp == -1)
         $timeStamp = time(); 
