...
require_once 'pbr-lib-common/src/Hadoop/WebHdfs/Curl.php';
...
class WebHDFS {
	...
	public function open($path, $offset='', $length='', $bufferSize='') {
		...
		return Curl::getWithRedirect($url, array(CURLOPT_HEADER => false));
	}

    public function downloadFile($path, $saveTo, $offset='', $length='', $bufferSize='') {
		$url = $this->_buildUrl($path, array('op'=>'OPEN', 'offset'=>$offset, 'length'=>$length, 'buffersize'=>$bufferSize));
		return Curl::downloadFile($url, $saveTo);
	}
	...
	public function getFileStatus($path) {
		...
		return Curl::get($url, array(CURLOPT_HEADER => false));
	}
    ...
    public function createUrlForHttpDownload($path) {
        return $this->_buildUrl($path, array('op'=>'OPEN'));
    }
